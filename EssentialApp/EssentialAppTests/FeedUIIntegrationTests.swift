...
class FeedUIIntegrationTests: XCTestCase {
	...
	func test_loadFeedACtions_requestFeedFromLoader() {
		...
		XCTAssertEqual(loader.loadFeedCallCount, 0, "Expected no loading requests before view is loader")
		
		sut.loadViewIfNeeded()
		XCTAssertEqual(loader.loadFeedCallCount, 1, "Expected a loading request once view is loader")
		...
		sut.simulateUserInitiatedReload()
		XCTAssertEqual(loader.loadFeedCallCount, 3, "Expected a thire loading request once user initiates another load")
	}
	...
	func test_loadMoreActions_requestMoreFromLoader() {
		...
		XCTAssertEqual(loader.loadMoreCallCount, 0, "Expected no requests untile load more action")
	...
	func test_loadingMoreIndicator_isVisibleWhileLoadingMore() {
		...
		XCTAssertFalse(sut.isShowingLoadMoreFeedIndicator, "Expected no loading indicator once user initiated loading is completed successfully")
		
		sut.simulateLoadMoreFeedAction()
		XCTAssertTrue(sut.isShowingLoadMoreFeedIndicator, "Expected loading indicator on load more action")
		
		loader.completeLoadMoreWithError(at: 1)
		XCTAssertFalse(sut.isShowingLoadMoreFeedIndicator, "Expected no loading indicator once user initiated loading is completed with error")
	}
	...
	func test_tapOnLoadMoreErrorView_hidesErrorMessage() {
		let (sut, loader) = makeSUT()
		sut.loadViewIfNeeded()
		loader.completeFeedLoading()
	...
	func test_feedImageView_cancelsImageLoadingWhenNotVisibleAnymore() {
		...
		XCTAssertEqual(loader.cancelledImageURLs, [image0.url, image1.url], "Expected second image URL request once second image is not visible anymore")
	}
	...
	func test_feedImageViewLoadingIndicator_isVisibleWhileLoadingImage() {
		...
		XCTAssertEqual(view0?.isShowingImageLoadingIndicator, false, "Expected loading no indicator for first view once first image loading completes successfully")
		XCTAssertEqual(view1?.isShowingImageLoadingIndicator, true, "Expected loading no indicator state change for second view once first image loading completes successfully")
		
		loader.completeImageLoadingWithError(at: 1)
		XCTAssertEqual(view0?.isShowingImageLoadingIndicator, false, "Expected loading no indicator state change for first view once second image loading completes with error")
		XCTAssertEqual(view1?.isShowingImageLoadingIndicator, false, "Expected loading no indicator state change for second view once second image loading completes with error")
	}
	...
	func test_feedImageView_rendersImageLoadedFromURL() {
		...
		XCTAssertEqual(view1?.renderedImage, .none, "Expected no image indicator for second view while loading second image")
	...
	func test_loadFeedCompletion_dispatchesFromBackgroundToMainThread() {
		...
		let exp = expectation(description: "Wait for backgground queue")
	...
	func test_feedImageView_doesNotLoaImageAgainUntilPreviousRequestCompletes() {
