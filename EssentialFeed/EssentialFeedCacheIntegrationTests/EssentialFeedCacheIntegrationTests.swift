...
final class EssentialFeedCacheIntegrationTests: XCTestCase {
	...
	func test_loadFeed_deliversItemsSaveOnASeparateInstance() {
	...
	private func save(_ data: Data, for url: URL, with loader: LocalFeedImageDataLoader, file: StaticString = #file, line: UInt = #line) {
		do {
			...
			XCTFail("Expected to save iamge adata successfully, got error \(error)", file: file, line: line)
		}
	}
	...
	private func expect(_ sut: LocalFeedImageDataLoader, toLoad expectedData: Data, for url: URL, file: StaticString = #file, line: UInt = #line) {
		...
		var loadResult = Result { try sut.loadImageData(from: url) }
		switch loadResult {
		case let .success(loadedData):
			XCTAssertEqual(loadedData, expectedData, file: file, line: line)
			
		case let .failure(error):
			XCTFail("Expected successful image data result, got \(error) isntead", file: file, line: line)
		}
	}
